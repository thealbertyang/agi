...
import type { ContentPart, Prompt, ImageAttachmentPart } from '@/context/prompt'
import { DEFAULT_PROMPT, isPromptEqual, usePrompt } from '@/context/prompt'
...
export const PromptInput: Component<PromptInputProps> = (props) => {
	...
	const handleKeyDown = (event: KeyboardEvent) => {
		...
		if (event.key === 'ArrowUp' || event.key === 'ArrowDown') {
			...
			const allowUp =
				isEmpty || atStart || (!hasNewlines && !inHistory) || (inHistory && atEnd)
			const allowDown =
				isEmpty || atEnd || (!hasNewlines && !inHistory) || (inHistory && atStart)
					...
					<Switch>
						<Match when={store.popover === 'file'}>
							<Show
								when={flat().length > 0}
								fallback={
									<div class="text-text-weak px-2 py-1">No matching files</div>
								}
							>
								<For each={flat()}>
									{(i) => (
						...
						<Match when={store.popover === 'slash'}>
							<Show
								...
								fallback={
									<div class="text-text-weak px-2 py-1">No matching commands</div>
								}
							>
								<For each={slashFlat()}>
									{(cmd) => (
					...
					<div class="flex items-center gap-1 absolute right-2 bottom-2">
						...
						>
							<IconButton
								...
								disabled={
									!prompt.dirty() &&
									store.imageAttachments.length === 0 &&
									!working()
								}
...
function setCursorPosition(parent: HTMLElement, position: number) {
	...
	while (node) {
		...
		const isFile =
			node.nodeType === Node.ELEMENT_NODE && (node as HTMLElement).dataset.type === 'file'
