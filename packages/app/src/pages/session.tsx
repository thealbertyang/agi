...
export default function Page() {
	...
	const working = createMemo(
		() => status().type !== 'idle' && activeMessage()?.id === lastUserMessage()?.id
	)
						...
						closeButton={
							terminal.all().length > 1 && (
								<IconButton
									icon="close"
									variant="ghost"
									onClick={() => terminal.close(props.terminal.id)}
								/>
							)
						}
					>
	...
	const showTabs = createMemo(
		() => layout.review.opened() && (diffs().length > 0 || tabs().all().length > 0)
	)
	...
	const MobileTurns = () => (
		...
		>
			<div
				ref={mobileAutoScroll.contentRef}
				class="flex flex-col gap-45 items-start justify-start mt-4"
			>
					...
					{(message) => (
						<SessionTurn
							...
							onStepsExpandedToggle={() =>
								setStore('mobileStepsExpanded', message.id, (x) => !x)
							}
							onUserInteracted={() => setStore('userInteracted', true)}
							classes={{
					...
					<Match when={diffs().length > 0}>
						<Tabs class="flex-1 min-h-0 flex flex-col pb-28">
							...
							<Tabs.Content
								forceMount
								value="review"
								class="flex-1 !overflow-hidden hidden data-[selected]:block"
							>
						...
						>
							...
							>
								...
								<Show when={diffs().length}>
									<Tabs.Content
										value="review"
										class="select-text flex flex-col h-full overflow-hidden contain-strict"
									>
							...
							<Tabs.List class="h-10">
								<SortableProvider ids={terminal.all().map((t: LocalPTY) => t.id)}>
									<For each={terminal.all()}>
										{(pty) => <SortableTerminalTab terminal={pty} />}
									</For>
								</SortableProvider>
							...
							<Show when={store.activeTerminalDraggable}>
								{(draggedId) => {
									const pty = createMemo(() =>
										terminal.all().find((t: LocalPTY) => t.id === draggedId())
									)
									return (
										<Show when={pty()}>
											{(t) => (
