...
export default function Layout(props: ParentProps) {
	...
	function navigateSessionByOffset(offset: number) {
		...
		const projectIndex = currentDirectory
			? projects.findIndex((p) => p.worktree === currentDirectory)
			: -1
		...
		const targetSession =
			offset > 0
				? nextProjectSessions[0]
				: nextProjectSessions[nextProjectSessions.length - 1]
	...
	const ProjectAvatar = (props: {
		...
		const mask =
			'radial-gradient(circle 5px at calc(100% - 2px) 2px, transparent 5px, black 5.5px)'
		...
		return (
			<div class="relative size-5 shrink-0 rounded-sm">
				<Avatar
					fallback={name()}
					src={
						props.project.id === opencode
							? 'https://opencode.ai/favicon.svg'
							: props.project.icon?.url
					}
	...
	const SessionItem = (props: {
		...
		const isWorking = createMemo(() => {
			...
			const status = globalSync.child(props.project.worktree)[0].session_status[
				props.session.id
			]
	...
	const ProjectDragOverlay = (): JSX.Element => {
		const project = createMemo(() =>
			layout.projects.list().find((p) => p.worktree === store.activeDraggable)
		)
		return (
			<Show when={project()}>
				{(p) => (
						...
						>
							...
							>
								<div class="relative -ml-px flex items-center justify-center size-4 [&>*]:absolute [&>*]:inset-0">
									<Icon
										name={
											layout.sidebar.opened() ? 'layout-left' : 'layout-right'
										}
										size="small"
						...
						<Match when={!providers.paid().length && expanded()}>
							<div class="rounded-md bg-background-stronger shadow-xs-border-base">
								<div class="p-3 flex flex-col gap-2">
									...
									<div class="text-text-base">
										Connect any provider to use models, inc. Claude, GPT, Gemini
										etc.
									</div>
								</div>
								<Tooltip
		...
		<div class="relative flex-1 min-h-0 flex flex-col">
			...
			<div class="flex-1 min-h-0 flex">
				<div
					...
					style={{
						width: layout.sidebar.opened() ? `${layout.sidebar.width()}px` : undefined,
					}}
				>
					<Show when={layout.sidebar.opened()}>
						<ResizeHandle
