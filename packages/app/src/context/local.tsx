...
export const { use: useLocal, provider: LocalProvider } = createSimpleContext({
	...
	init: () => {
		...
		const model = (() => {
			...
			const latest = createMemo(() =>
				pipe(
					available(),
					filter(
						(x) => Math.abs(DateTime.fromISO(x.release_date).diffNow().as('months')) < 6
					),
		...
		const file = (() => {
			...
			const open = async (
				path: string,
				options?: { pinned?: boolean; view?: LocalFile['view'] }
			) => {
				const relativePath = relative(path)
				if (!store.node[relativePath]) await fetch(path)
				// setStore("opened", (x) => {
				...
				if (options?.view && store.node[relativePath].view === undefined)
					setStore('node', path, 'view', options.view)
				if (store.node[relativePath]?.loaded) return
				return load(relativePath)
			}
			...
			const searchFiles = (query: string) =>
				sdk.client.find.files({ query, dirs: 'false' }).then((x) => x.data!)
		...
		const context = (() => {
			...
			const activeFile = createMemo(() =>
				store.activeFile ? file.node(store.activeFile) : undefined
			)
