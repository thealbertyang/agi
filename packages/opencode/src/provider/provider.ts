...
export namespace Provider {
	...
	const CUSTOM_LOADERS: Record<string, CustomLoader> = {
			...
			return {
				...
				async getModel(sdk: any, modelID: string, _options?: Record<string, any>) {
					...
					switch (regionPrefix) {
						...
						case 'ap': {
							const isAustraliaRegion = ['ap-southeast-2', 'ap-southeast-4'].includes(
								region
							)
							if (
								isAustraliaRegion &&
		...
		'google-vertex': async () => {
			...
			const location =
				Env.get('GOOGLE_CLOUD_LOCATION') ?? Env.get('VERTEX_LOCATION') ?? 'us-east5'
		...
		'google-vertex-anthropic': async () => {
			const project =
				...
				Env.get('GCLOUD_PROJECT')
			const location =
				Env.get('GOOGLE_CLOUD_LOCATION') ?? Env.get('VERTEX_LOCATION') ?? 'global'
			const autoload = Boolean(project)
			if (!autoload) return { autoload: false }
			return {
				autoload: true,
		...
		for (const [providerID, provider] of configProviders) {
			...
			for (const [modelID, model] of Object.entries(provider.models ?? {})) {
				...
				const parsedModel: Model = {
					...
					capabilities: {
						temperature:
							model.temperature ?? existingModel?.capabilities.temperature ?? false,
						reasoning:
							model.reasoning ?? existingModel?.capabilities.reasoning ?? false,
						attachment:
							model.attachment ?? existingModel?.capabilities.attachment ?? false,
					...
					cost: {
						...
						cache: {
							...
							write:
								model?.cost?.cache_write ?? existingModel?.cost?.cache.write ?? 0,
						},
					},
