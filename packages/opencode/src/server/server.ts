			...
			.get(
				...
				async (c) => {
					...
					const providers = await Provider.list().then((x) =>
						mapValues(x, (item) => item)
					)
					return c.json({
						providers: Object.values(providers),
						default: mapValues(
							providers,
							(item) => Provider.toSorted(Object.values(item.models))[0].id
						),
					})
				}
			)
					...
					responses: {
						200: {
							...
							content: {
								'application/json': {
									schema: resolver(
										z.record(z.string(), z.array(ProviderAuth.Method))
									),
								},
							},
						},
				...
				validator(
					...
					z.object({
						...
						code: z
							.string()
							.optional()
							.meta({ description: 'OAuth authorization code' }),
					})
				),
				async (c) => {
					const providerID = c.req.valid('param').providerID
