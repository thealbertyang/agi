...
import { addDefaultParsers, MacOSScrollAccel, type ScrollAcceleration } from '@opentui/core'
import {
	useKeyboard,
	useRenderer,
...
export function Session() {
	...
	const [userMessageMarkdown, setUserMessageMarkdown] = createSignal(
		kv.get('user_message_markdown', true)
	)
	...
	command.register(() => [
		...
		{
			title: userMessageMarkdown()
				? 'Disable user message markdown'
				: 'Enable user message markdown',
													...
													>
														...
														>
															<text fg={theme.textMuted}>
																{revert()!.reverted.length} message
																reverted
															</text>
															<text fg={theme.textMuted}>
...
function UserMessage(props: {
	...
	const color = createMemo(() =>
		queued() ? theme.accent : local.agent.color(props.message.agent)
	)
...
function ToolPart(props: { last: boolean; part: ToolPart; message: AssistantMessage }) {
	...
	const component = createMemo(() => {
		...
		const shouldHide =
			...
			!sync.data.permission[props.message.sessionID]?.some(
				(x) => x.callID === props.part.callID
			)
...
function ToolTitle(props: { fallback: string; when: any; icon: string; children: JSX.Element }) {
	...
	return (
		<text
			paddingLeft={3}
			fg={props.when ? theme.textMuted : theme.text}
		>
			<Show
