...
export function DialogModel(props: { providerID?: string }) {
	...
	const options = createMemo(() => {
			...
			? recents
					.filter(
						(item) =>
							!favorites.some(
								(fav) =>
									fav.providerID === item.providerID &&
									fav.modelID === item.modelID
							)
					)
					.slice(0, 5)
			: []
		...
		if (q) {
			const filteredFavorites = fuzzysort
				.go(q, favoriteOptions, { keys: ['title'] })
				.map((x) => x.obj)
			const filteredRecents = fuzzysort
				.go(q, recentOptions, { keys: ['title'] })
				.map((x) => x.obj)
			const filteredProviders = fuzzysort
				.go(q, providerOptions, { keys: ['title', 'category'] })
				.map((x) => x.obj)
			const filteredPopular = fuzzysort
				.go(q, popularProviders, { keys: ['title'] })
				.map((x) => x.obj)
			return [
				...filteredFavorites,
				...filteredRecents,
				...
				},
				{
					...
					onTrigger: (option) => {
						local.model.toggleFavorite(
							option.value as { providerID: string; modelID: string }
						)
					},
				},
			]}
