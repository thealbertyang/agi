...
export const GithubCommand = cmd({
	...
	builder: (yargs) =>
		yargs.command(GithubInstallCommand).command(GithubRunCommand).demandCommand(),
	async handler() {},
})
			...
			async fn() {
				{
					...
					async function getAppInfo() {
						...
						// Get repo info
						const info = (
							await $`git remote get-url origin`.quiet().nothrow().text()
						).trim()
...
export const GithubRunCommand = cmd({
	...
	async handler(args) {
		await bootstrap(process.cwd(), async () => {
			...
			if (
				!SUPPORTED_EVENTS.includes(context.eventName as (typeof SUPPORTED_EVENTS)[number])
			) {
			...
			const issueId = isScheduleEvent
				...
				: context.eventName === 'issue_comment'
					...
					: (payload as PullRequestEvent | PullRequestReviewCommentEvent).pull_request
							.number
			...
			async function getUserPrompt() {
				...
				let prompt = (() => {
					...
					throw new Error(
						`Comments must mention ${mentions.map((m) => '`' + m + '`').join(' or ')}`
					)
				})()
				...
				const matches = [...mdMatches, ...tagMatches].toSorted((a, b) => a.index - b.index)
			...
			async function configureGit(appToken: string) {
				...
				const newCredentials = Buffer.from(`x-access-token:${appToken}`, 'utf8').toString(
					'base64'
				)
...
Co-authored-by: ${actor} <${actor}@users.noreply.github.com>"`
			...
			async function removeReaction(commentType?: 'issue' | 'pr_review') {
				...
				if (triggerCommentId) {
					if (commentType === 'pr_review') {
						const reactions =
							await octoRest.rest.reactions.listForPullRequestReviewComment({
								owner,
								repo,
								comment_id: triggerCommentId!,
								content: AGENT_REACTION,
			...
			function footer(opts?: { image?: boolean }) {
				const image = (() => {
					...
					const title64 = Buffer.from(session.title.substring(0, 700), 'utf8').toString(
						'base64'
					)
