	...
	async handler(args) {
		await Instance.provide({
			...
			async fn() {
				...
				const result =
					await $`gh pr checkout ${prNumber} --branch ${localBranchName} --force`.nothrow()
				...
				if (prInfoResult.exitCode === 0) {
					...
					if (prInfoText.trim()) {
						...
						if (
							prInfo &&
							prInfo.isCrossRepository &&
							prInfo.headRepository &&
						...
						if (prInfo && prInfo.body) {
							const sessionMatch = prInfo.body.match(
								/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/
							)
							if (sessionMatch) {
								...
								const importResult =
									await $`opencode import ${sessionUrl}`.nothrow()
								if (importResult.exitCode === 0) {
									const importOutput = importResult.text().trim()
									// Extract session ID from the output (format: "Imported session: <session-id>")
									const sessionIdMatch = importOutput.match(
										/Imported session: ([a-zA-Z0-9_-]+)/
									)
									if (sessionIdMatch) {
				...
				UI.println(
					`Successfully checked out PR #${prNumber} as branch '${localBranchName}'`
				)
				UI.println()
