    ...
    "my-server": {
      ...
      "url": "https://example.com/mcp"
				...
				if (serverConfig.type !== 'remote' || serverConfig.oauth === false) {
					prompts.log.error(
						`MCP server ${serverName} does not support OAuth (oauth is disabled)`
					)
					prompts.outro('Done')
					return
				}
		...
		if (type === 'remote') {
			...
			if (useOAuth) {
				...
				if (hasClientId) {
					...
					prompts.log.info(
						`Remote MCP server "${name}" configured with OAuth (client ID: ${clientId})`
					)
    ...
    "${name}": {
      ...
      "oauth": {
        "clientId": "${clientId}"${clientSecret ? `,\n        "clientSecret": "${clientSecret}"` : ''}
					...
					prompts.log.info(
						`Remote MCP server "${name}" configured with OAuth (dynamic registration)`
					)
					prompts.log.info('Add this to your opencode.json:')
					prompts.log.info(`
  "mcp": {
	...
	async handler(args) {
		await Instance.provide({
			...
			async fn() {
				...
				if (entry?.tokens) {
					prompts.log.info(
						`  Access token: ${entry.tokens.accessToken.substring(0, 20)}...`
					)
					...
					if (response.status === 401) {
						...
						try {
							...
							if (json.result?.serverInfo) {
								prompts.log.info(
									`Server info: ${JSON.stringify(json.result.serverInfo)}`
								)
							}
						} catch {
							// Not JSON, ignore
