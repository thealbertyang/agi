	...
	export class Agent implements ACPAgent {
					...
					switch (event.type) {
						case 'permission.updated':
							try {
								...
								log.error('unexpected error when handling permission', {
									error: err,
								})
							} finally {
								break
						...
						case 'message.part.updated':
							...
							try {
								...
								const message = await this.config.sdk.session
									...
									.then((x) => x.data)
									.catch((err) => {
										log.error('unexpected error when fetching message', {
											error: err,
										})
										return undefined
									})
								...
								if (part.type === 'tool') {
									switch (part.state.status) {
											...
											if (part.tool === 'todowrite') {
												...
												if (parsedTodos.success) {
													await this.connection
														...
														.catch((err) => {
															log.error(
																'failed to send session update for todo',
																{ error: err }
															)
														})
												} else {
													log.error('failed to parse todo output', {
														error: parsedTodos.error,
													})
												}
											}
										...
										case 'error':
											await this.connection
												...
												.catch((err) => {
													log.error('failed to send tool error to ACP', {
														error: err,
													})
												})
											break
									...
									if (delta && part.synthetic !== true) {
										await this.connection
											...
											.catch((err) => {
												log.error('failed to send text to ACP', {
													error: err,
												})
											})
									}
									...
									if (delta) {
										await this.connection
											...
											.catch((err) => {
												log.error('failed to send reasoning to ACP', {
													error: err,
												})
											})
									}
		...
		async loadSession(params: LoadSessionRequest) {
			...
			try {
				...
				// Store ACP session state
				const state = await this.sessionManager.load(
					sessionId,
					params.cwd,
					params.mcpServers,
					model
					...
					switch (part.state.status) {
						case 'pending':
							await this.connection
								...
								.catch((err) => {
									log.error('failed to send tool pending to ACP', {
										error: err,
									})
								})
							break
						case 'running':
							await this.connection
								...
								.catch((err) => {
									log.error('failed to send tool in_progress to ACP', {
										error: err,
									})
								})
							break
						case 'completed':
							...
							if (part.tool === 'todowrite') {
								...
								if (parsedTodos.success) {
									await this.connection
										...
										.catch((err) => {
											log.error('failed to send session update for todo', {
												error: err,
											})
										})
								} else {
							...
							await this.connection
								...
								.catch((err) => {
									log.error('failed to send tool completed to ACP', {
										error: err,
									})
								})
							break
		...
		private async loadSessionMode(params: LoadSessionRequest) {
			...
			const entries = providers.toSorted((a, b) => {
			...
			const availableModels = entries.flatMap((provider) => {
				const models = Provider.toSorted(Object.values(provider.models))
			...
			const currentModeId =
				availableModes.find((m) => m.name === defaultAgentName)?.id ?? availableModes[0].id
