	...
	export const Rubocop: Info = {
		...
		async spawn(root) {
			...
			if (!bin) {
				...
				bin = path.join(
					Global.Path.bin,
					'rubocop' + (process.platform === 'win32' ? '.exe' : '')
				)
		...
		async spawn(root) {
			...
			if (!bin) {
				...
				if (!supportedCombos.includes(assetName)) {
					log.error(
						`Platform ${platform} and architecture ${arch} is not supported by zls`
					)
					return
				}
	...
	export const CSharp: Info = {
		...
		async spawn(root) {
			...
			if (!bin) {
				...
				bin = path.join(
					Global.Path.bin,
					'csharp-ls' + (process.platform === 'win32' ? '.exe' : '')
				)
	...
	export const Clangd: Info = {
		...
		async spawn(root) {
			...
			const entries = await fs
				.readdir(Global.Path.bin, { withFileTypes: true })
				.catch(() => [])
	...
	export const LuaLS: Info = {
		...
		async spawn(root) {
			...
			if (!bin) {
				...
				if (!asset) {
					log.error(
						`Could not find asset ${assetName} in latest lua-language-server release`
					)
					return
				}
				...
				if (platform !== 'win32') {
					const ok = await $`chmod +x ${bin}`.quiet().catch((error) => {
						log.error(
							'Failed to set executable permission for lua-language-server binary',
							{
								error,
							}
						)
	...
	export const TerraformLS: Info = {
		...
		async spawn(root) {
			...
			if (!bin) {
				...
				bin = path.join(
					Global.Path.bin,
					'terraform-ls' + (platform === 'win32' ? '.exe' : '')
				)
		...
		async spawn(root) {
			...
			if (!binary) {
				...
				if (!(await Bun.file(js).exists())) {
					if (Flag.OPENCODE_DISABLE_LSP_DOWNLOAD) return
					await Bun.spawn(
						[BunProc.which(), 'install', 'dockerfile-language-server-nodejs'],
						{
							cwd: Global.Path.bin,
							env: {
								...process.env,
		...
		async spawn(root) {
			...
			if (!bin) {
				...
				if (ext === 'zip') {
					...
					await $`tar -xzf ${tempPath} --strip-components=1`
						.cwd(Global.Path.bin)
						.quiet()
						.nothrow()
				}
