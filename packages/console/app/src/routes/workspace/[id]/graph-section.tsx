			...
			tx
				...
				.where(
					and(
						...
						usageKeyIds.size > 0
							? or(
									inArray(KeyTable.id, Array.from(usageKeyIds)),
...
export function GraphSection() {
	...
	const getModels = createMemo(() => {
		...
		return Array.from(new Set(store.data.usage.map((row) => row.model))).toSorted()
	})
	...
	const formatMonthYear = () =>
		new Date(store.year, store.month, 1).toLocaleDateString('en-US', {
			month: 'long',
			year: 'numeric',
		})
							...
							chart.data.datasets?.forEach((dataset, i) => {
								...
								const color =
									i === legendItem.datasetIndex
										? baseColor
										: addOpacityToColor(baseColor, 0.3)
