	...
	const toImg = (src: any) => {
		...
		if (
			...
			return {
				type: 'image_url',
				image_url: {
					url: `data:${(src as any).media_type};base64,${(src as any).data}`,
				},
			}
		return undefined
	}
	...
	const tc = (() => {
		...
		if ((tcin as any).type === 'tool' && typeof (tcin as any).name === 'string')
			return {
				...
				function: { name: (tcin as any).name },
			}
		return undefined
	})()
...
export function fromAnthropicResponse(resp: any): CommonResponse {
	...
	const usage = (() => {
		...
		const ct =
			typeof (u as any).output_tokens === 'number' ? (u as any).output_tokens : undefined
	...
	if (json.type === 'content_block_delta') {
		...
		if (d?.type === 'text_delta') {
			...
			out.choices.push({
				...
				delta: {
					tool_calls: [
						{ index: json.index ?? 0, function: { arguments: d.partial_json } },
					],
				},
				finish_reason: null,
			})
