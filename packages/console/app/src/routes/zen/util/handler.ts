...
export async function handler(
		...
		const provider = (() => {
			if (authInfo?.provider?.credentials) {
				return modelInfo.providers.find(
					(provider) => provider.id === modelInfo.byokProvider
				)
			}

			if (isTrial) {
				return modelInfo.providers.find(
					(provider) => provider.id === modelInfo.trial!.provider
				)
			}

			if (stickyProvider) {
				const provider = modelInfo.providers.find(
					(provider) => provider.id === stickyProvider
				)
			...
			tx
				...
				.leftJoin(
					...
					and(
						...
						eq(ModelTable.model, modelInfo.id)
					)
				)
				.leftJoin(
					ProviderTable,
	...
	async function trackUsage(
		...
		const cost =
			authInfo.isFree || authInfo.provider?.credentials
				? 0
				: centsToMicroCents(totalCostInCent)
