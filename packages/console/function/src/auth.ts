					...
					const matches = await Database.use(async (tx) =>
						tx
							...
							.where(
								or(
									and(
										eq(AuthTable.provider, response.provider),
										eq(AuthTable.subject, subject)
									),
									and(
										eq(AuthTable.provider, 'email'),
										eq(AuthTable.subject, email)
									)
								)
							)
					)
					const idByProvider = matches.find(
						(x) => x.provider === response.provider
					)?.accountID
