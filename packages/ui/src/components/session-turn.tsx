...
function AssistantMessageItem(props: {
	...
	const filteredParts = createMemo(() => {
		if (
			...
			!props.summary &&
			props.response &&
			props.lastTextPartId === lastTextPart()?.id
		) {
...
export function SessionTurn(
	...
	const assistantMessages = createMemo(() => {
		...
		return allMessages().filter(
			(m) => m.role === 'assistant' && m.parentID === msg.id
		) as AssistantMessage[]
	})
	...
	const rawStatus = createMemo(() => {
		...
		if (
			currentTask?.state &&
			'metadata' in currentTask.state &&
			currentTask.state.metadata?.sessionId
		) {
			const taskMessages = data.store.message[
				currentTask.state.metadata.sessionId as string
	...
	function duration() {
		...
		const unit: DurationUnit[] =
			interval.length('seconds') > 60 ? ['minutes', 'seconds'] : ['seconds']
							...
							>
								<Switch>
									...
									<Match when={true}>
										{/* Title (sticky) */}
										<div
											ref={(el) => setStore('stickyTitleRef', el)}
														...
														<Match when={retry()}>
															...
															<span data-slot="session-turn-retry-seconds">
																...
																{store.retrySeconds > 0
																	? `in ${store.retrySeconds}s `
																	: ''}
															</span>
															<span data-slot="session-turn-retry-attempt">
																(#{retry()?.attempt})
															</span>
														</Match>
														<Match when={waitingForSummary()}>
															Generating summary
														</Match>
														<Match when={working()}>
															{store.status ??
																		...
																		<div data-slot="session-turn-accordion-trigger-content">
																			<div data-slot="session-turn-file-info">
																				...
																				<div data-slot="session-turn-file-path">
																					...
																					<span data-slot="session-turn-filename">
																						{getFilename(
																							diff.file
																						)}
																					</span>
