...
import type { FileDiff, Message, Model, Part, Session } from '@opencode-ai/sdk/v2'
...
export namespace Share {
	...
	export const sync = fn(
		...
		async (input) => {
			...
			await Storage.write(
				['share_event', input.share.id, Identifier.descending()],
				input.data
			)
		}
	)
				...
				promises.push(
					iife(async () => {
						switch (item.type) {
							case 'session':
								await Storage.write(
									['share_data', input.share.id, 'session'],
									item.data
								)
								break
							case 'message': {
								...
								await Storage.write(
									['share_data', input.share.id, 'message', data.id],
									item.data
								)
								break
							}
							...
							case 'session_diff':
								await Storage.write(
									['share_data', input.share.id, 'session_diff'],
									item.data
								)
								break
							case 'model':
								await Storage.write(
									['share_data', input.share.id, 'model'],
									item.data
								)
								break
						}
