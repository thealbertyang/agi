...
const ClientOnlyDiff = clientOnly(() =>
	import('@opencode-ai/ui/diff').then((m) => ({ default: m.Diff }))
)
const ClientOnlyCode = clientOnly(() =>
	import('@opencode-ai/ui/code').then((m) => ({ default: m.Code }))
)
				...
				{(data) => {
					...
					const ogImage = createMemo(() => {
						...
						if (modelIDs.length === 1) {
							...
							modelParam = encodeURIComponent(
								`${modelIDs[0]} & ${modelIDs.length - 1} others`
							)
						} else {
							modelParam = 'unknown'
						}
										...
										>
											{iife(() => {
												...
												const provider = createMemo(
													() => activeMessage()?.model?.providerID
												)
												const modelID = createMemo(
													() => activeMessage()?.model?.modelID
												)
												const model = createMemo(() =>
													data().model[data().sessionID]?.find(
														(m) => m.id === modelID()
													)
												)
												const diffs = createMemo(() => {
													...
													const preloaded =
														data().session_diff_preload[
															data().sessionID
														] ?? []
													return diffs.map((diff) => ({
														...diff,
														preloaded: preloaded.find(
															(d) => d.newFile.name === diff.file
														),
													}))
												})
												const splitDiffs = createMemo(() => {
													const diffs =
														data().session_diff[data().sessionID] ?? []
													const preloaded =
														data().session_diff_preload_split[
															data().sessionID
														] ?? []
													return diffs.map((diff) => ({
														...diff,
													...
													<div class="flex flex-col gap-4">
														<div class="flex flex-col gap-2 sm:flex-row sm:gap-4 sm:items-center sm:h-8 justify-start self-stretch">
															<div class="pl-[2.5px] pr-2 flex items-center gap-1.75 bg-surface-strong shadow-xs-border-base w-fit">
																...
																<div class="text-12-mono text-text-base">
																	v{info().version}
																</div>
															</div>
