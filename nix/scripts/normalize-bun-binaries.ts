...
const bunEntries = (await safeReadDir(bunRoot)).toSorted()
let rewritten = 0
...
for (const entry of bunEntries) {
	...
	for (const packageDir of packageDirs) {
		...
		if (typeof binField === 'string') {
			...
			const entries = Object.entries(binField).toSorted((a, b) => a[0].localeCompare(b[0]))
...
async function collectPackages(modulesRoot: string) {
	const found: string[] = []
	const topLevel = (await safeReadDir(modulesRoot)).toSorted()
	for (const name of topLevel) {
		...
		if (name.startsWith('@')) {
			const scoped = (await safeReadDir(full)).toSorted()
